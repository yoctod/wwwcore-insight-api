...
var bitcore = require('wwwcore-lib');
...
BlockController.prototype.block = function(req, res, next) {
  ...
  if (blockCached) {
    blockCached.confirmations = self.node.services.wwwcoind.height - blockCached.height + 1;
    ...
    self.node.getBlock(hash, function(err, block) {
      ...
      self.node.services.wwwcoind.getBlockHeader(hash, function(err, info) {
        if (err) {
          return self.common.handleErrors(err, res);
        }
...
BlockController.prototype.blockIndex = function(req, res) {
  ...
  this.node.services.wwwcoind.getBlockHeader(parseInt(height), function(err, info) {
    if (err) {
      return self.common.handleErrors(err, res);
    }
    res.jsonp({
...
BlockController.prototype._getBlockSummary = function(hash, moreTimestamp, next) {
  ...
  if (summaryCache) {
    ...
    self.node.services.wwwcoind.getRawBlock(hash, function(err, blockBuffer) {
      if (err) {
        return next(err);
      }
      ...
      self.node.services.wwwcoind.getBlockHeader(hash, function(err, blockHeader) {
        if (err) {
          return next(err);
        }
        ...
        var confirmations = self.node.services.wwwcoind.height - height + 1;
...
BlockController.prototype.list = function(req, res) {
  ...
  self.node.services.wwwcoind.getBlockHashesByTimestamp(lte, gte, function(err, hashes) {
    if(err) {
      return self.common.handleErrors(err, res);
    }
...
BlockController.prototype.getBlockReward = function(height) {
  var halvings = Math.floor(height / 1680000);
  // Force block reward to zero when right shift is undefined.
  if (halvings >= 64) {
    return 0;
  }

  // Subsidy is cut in half every 1,680,000 blocks which will occur approximately every 4 years.
